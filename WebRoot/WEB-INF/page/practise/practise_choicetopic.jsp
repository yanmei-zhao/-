...
<script type="text/javascript">
  ...
  <body>
				  	    ...
				  	    <s:iterator value="pageResult1.data" id="id"> 
					  	  <div class="view_box">
						  	  	...
						  	  	<div class="view_answer">
						  	  		<ul>
						  	  			<div class="radio">
						  	  				<label><input name="answer_${id }" type="radio" value="A"/>A.${optionA } </label>
						  	  			</div>
						  	  			<div class="radio">
						  	  				<label><input name="answer_${id }" type="radio" value="B"/>B.${optionB }</label>
						  	  			</div>
						  	  			<div class="radio">
						  	  				<label><input name="answer_${id }" type="radio" value="C"/>C.${optionC }</label>
						  	  			</div>
						  	  			<div class="radio">
						  	  				<label><input name="answer_${id }" type="radio" value="D"/>D.${optionD }</label>
						  	  			</div>
						  	  		</ul>
						  	  	</div>
						  	  	<div class="view_description">
    ...
    <div class="tip">
	    ...
	    <!-- 分页菜单组件--------------------------开始 -->
	<%//查询的url地址，统一写
	String listActionURL = basePath+"/front/ChoiceTopic_practiseList.action";
	%>
	<script type="text/javascript">
	//分页组件
	function change()
	  {
	  var url = "<%= basePath%>/front/ChoiceTopic_practiseList.action";                 //获取表单url
