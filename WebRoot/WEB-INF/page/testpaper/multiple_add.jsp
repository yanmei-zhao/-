...
<script type="text/javascript">
		...
		var tmBatch = {
			...
			addBranches : function(){
				$("input[name='checkbox']").each(function(idx, item){
					var multipleId = $(item).attr("multipleId");
					var description = $(item).attr("description");
					var is_me_checked = $(item).is(':checked') ;
					if(is_me_checked){
						tmBatch.addOneBranch(multipleId, description);
					}else{
						tmBatch.removeOneBranch(multipleId);
					}
				});
			...
			addOneBranch : function(multipleId, description){
				var isExist = tmBatch.branchExsit(multipleId);
				if(!isExist){
					var shtml = [];
					shtml.push('<div class="choice">');
					shtml.push('分值：');
					shtml.push('<input type="text" name="multiplescore" class="tm_qscore"/>');
					shtml.push('<input type="hidden" name="multipleId" value="'+multipleId+'" /> ' + description );
					shtml.push('<a href="javascript:void(0);" onclick="javascript:tm_removeBranch(this)"><img src="<%=path%>/images/no.png" /></a>');
					shtml.push('</div>');

					$('#mulTab', window.parent.document).append(shtml.join(""));
				}
			}, 

			doInsertBranch : function(obj){
				var multipleId = $(obj).attr("multipleId");
			...
			removeOneBranch : function(multipleId){
				$('#mulTab input[value='+multipleId+']', window.parent.document).parent().remove();
				window.parent.layer.msg('操作成功');
				window.location.reload();
			},
			...
			loadBranches : function(){
				$("a.tm-a-branches").each(function(idx, item){
					...
					var isExist = tmBatch.branchExsit(multipleId);
					if(isExist){
						$(item).html('<img src="<%=path%>/images/remove.png" />移除').attr("onclick","javascript:tmBatch.removeOneBranch('"+multipleId+"')").css({"color":"#f00"});
						$(item).parent().parent().children("td").first().html('<img src="<%=path%>/images/gou.png" />');
					}
				});
			},
			
			branchExsit : function(multipleId){
				return $('#mulTab input[value='+multipleId+']', window.parent.document).length>0;
			}
		};
	</script>
</head>
  ...
  <body>
    <div class="formbody">
	    <div id="usual1" class="usual">
	     	 <div id="tab2" class="tabson">
		     	<form action="<%=basePath%>/front/Testpaper_openMultipleTopicList.action" method="post">
			    	<ul class="seachform">
				    ...
				    <table class="tablelist ">
				        ...
				        <tbody>
				        <s:iterator value="pageResult5.data" id="id">
					        <tr>
						        <td><input name="checkbox" type="checkbox" multipleId="${id}" description="${description}"/></td>
						        <td>${id}</td>
						        <td>${description}</td>
						        <td>${topicBankName}</td>
						        <td>${type}</td>
						        ...
						        <td>
						            ...
						            <a href="javascript:void(0);" class="tm-a-branches" multipleId="${id}" description="${description}" onclick="javascript:tmBatch.doInsertBranch(this)"><img src="<%=path%>/images/add1.png" />选择</a>
						      </td>
					        </tr> 
				        </s:iterator>
				        </tbody>
	   ...
	   <div class="pagin">
    	...
    	共<i class="blue">${pageResult5.total}</i>条记录 	<i class="blue">${pageResult5.totalPage}</i>页， 	当前显示第&nbsp;<i class="blue">${pageResult5.page}</i>页</div>
        <ul class="paginList">
           <c:choose>
			   <c:when test="${pageResult5.isFirst==true}"><li class="paginItem current"><a href="javascript:;">首页</a></li></c:when>
		       <c:otherwise>
			       <li class="paginItem"><a href="javascript:first()" target="rightFrame">首页&nbsp;</a></li>
		       </c:otherwise>
		   </c:choose>
           <c:choose>
		      <c:when test="${pageResult5.isFirst==true}"><li class="paginItem current"><a href="javascript:;">上一页</a></li></c:when>
		      <c:otherwise>
			      <li class="paginItem"><a href="javascript:previous()" target="rightFrame">上一页&nbsp;</a></li>
		      </c:otherwise>
		   </c:choose>
           <c:choose>
			   <c:when test="${pageResult5.hasNext==true}">
           ...
           <c:choose>
			   <c:when test="${pageResult5.isLast==true}"><li class="paginItem current"><a href="javascript:;">尾页</a></li></c:when>
		       <c:otherwise>
	...
	<!-- 分页菜单组件--------------------------开始 -->
	<%//查询的url地址，统一写
	String listActionURL = basePath+"/front/Testpaper_openMultipleTopicList.action";
	%>
	<script type="text/javascript">
	//分页组件
	function change()
	  {
	  var url = "<%= basePath%>/front/Testpaper_openMultipleTopicList.action";                 //获取表单url
	 	var textfield=document.getElementById("textfield").value;
	 	var totalPage='${pageResult5.totalPage}';
	 	var pageNum = 0;
	...
	<script type="text/javascript">
		...
		//上一页
		function previous(){
		    window.location.href  = url+"?page=${pageResult5.previousPageNumber}";
		}
		//下一页
		function next(){
		    window.location.href  = url+"?page=${pageResult5.nextPageNumber}";
		}
		//尾页
		function last(){
		  window.location.href  = url+"?page=${pageResult5.totalPage}";
		}
	</script>
	<!-- 分页菜单组件--------------------------结束 -->
	<script type="text/javascript"> 
