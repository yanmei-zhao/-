...
//		return SUCCESS;
//	}
	public List gettopicBankName() throws Exception{
			HttpServletResponse response=ServletActionContext.getResponse();
			String topicBankType = topicBank.getTopicBankType();
			response.setContentType("text/html;charset=UTF-8"); 			// 回传内容设为json格式 2017.08.13 miki
		         
       		//根据一级菜单选中的Id，得到对应的二级列表
		    List<String> topicBankNameList1=topicBankService.gettopicBankName(topicBankType);
			//以jsonArray数据形式存入json中
			JSONArray topicBankName=JSONArray.fromObject(topicBankNameList1);	
				
			// * response顾名思义就是服务器对浏览器的相应，PrintWriter它的实例就是向前台的JSP页面输出结果
			 
			PrintWriter out=response.getWriter();								
			out.print(topicBankName.toString());
			out.flush();
			out.close();
