...
//		String queryString="from Exam where examState='"+examState+"' and className = '"+className+"' order by examStart desc,examId desc";
		String queryString="select distinct t0.examId ,t0.examName,t0.examStart,t0.examEnd,t0.examDuration from Exam t0,Student t1,ExamClass t2 where t0.examState='"+examState+"' and t1.classId = t2.classId and t2.examId = t0.examId and t1.classId="+classId;
		if(null !=exam.getExamId()){
			queryString = queryString +" and examId like '%"+exam.getExamId() +"%' "; 
		}
		if(null != exam.getExamName()){
